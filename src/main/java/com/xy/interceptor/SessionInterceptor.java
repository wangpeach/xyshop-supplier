...
public class SessionInterceptor extends HandlerInterceptorAdapter {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        Cookie cookie = CookieUtils.getCookieByName(request, CookieUtils.XY_TICKET);
        if (cookie == null) {
            this.goToLogin(request, response, "sessionInvalid");
            return false;
        }
        try {
            String loginShop = "_loginshop_";
            if (request.getSession().getAttribute(loginShop) == null) {
                String[] values = StringUtils.split(cookie.getValue(), CookieUtils.XY_TICKET_SEPERATOR);
                ...
                Thread.sleep(1000);
                request.getSession().setAttribute(loginShop, shop);
    ...
    private void goToLogin(HttpServletRequest request,
        ...
        RequestDispatcher dispatcher = request.getRequestDispatcher("/invalid");
